...
public class LagPlayerGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#pragma warning restore 649
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            var ghostLagPlayerArray = chunk.GetNativeArray(ghostLagPlayerType);
#if UNITY_EDITOR || DEVELOPMENT_BUILD
...
#endif
                LagPlayerSnapshotData snapshotData;
                snapshot.GetDataAtTick(targetTick, targetTickFraction, out snapshotData);

                var ghostLagPlayer = ghostLagPlayerArray[entityIndex];
                ghostLagPlayer.playerId = snapshotData.GetLagPlayerplayerId(deserializerState);
                ghostLagPlayerArray[entityIndex] = ghostLagPlayer;
            }
        }
    }
    [BurstCompile]
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<LagPlayerSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadWrite<LagPlayer>(),
            },
        ...
        {
            All = new []{
                ComponentType.ReadOnly<LagPlayerSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadOnly<PredictedGhostComponent>(),
                ComponentType.ReadWrite<LagPlayer>(),
            }
        });
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<LagPlayerSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
...
public partial class LagPlayerGhostSpawnSystem : DefaultGhostSpawnSystem<LagPlayerSnapshotData>
{
    struct SetPredictedDefault : IJobParallelFor
    {
        [ReadOnly] public NativeArray<LagPlayerSnapshotData> snapshots;
        ...
        {
            if (localPlayerId.Length == 1 && snapshots[index].GetLagPlayerplayerId() == localPlayerId[0].Value)
                predictionMask[index] = 1;
        }
    }
    protected override JobHandle SetPredictedGhostDefaults(NativeArray<LagPlayerSnapshotData> snapshots, NativeArray<int> predictionMask, JobHandle inputDeps)
    {
