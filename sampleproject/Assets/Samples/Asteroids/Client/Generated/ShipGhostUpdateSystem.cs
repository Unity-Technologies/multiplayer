...
#pragma warning restore 649
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            var ghostShipStateComponentDataArray = chunk.GetNativeArray(ghostShipStateComponentDataType);
            ...
            for (int entityIndex = 0; entityIndex < ghostEntityArray.Length; ++entityIndex)
            {
                ...
                var ghostShipStateComponentData = ghostShipStateComponentDataArray[entityIndex];
                var ghostRotation = ghostRotationArray[entityIndex];
                var ghostTranslation = ghostTranslationArray[entityIndex];
                ghostShipStateComponentData.State = snapshotData.GetShipStateComponentDataState(deserializerState);
                ...
                ghostShipStateComponentDataArray[entityIndex] = ghostShipStateComponentData;
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadOnly<ShipSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
        ...
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<ShipSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
...
public partial class ShipGhostSpawnSystem : DefaultGhostSpawnSystem<ShipSnapshotData>
{
    ...
    {
        [ReadOnly] public NativeArray<ShipSnapshotData> snapshots;
        ...
        {
            if (localPlayerId.Length == 1 && snapshots[index].GetPlayerIdComponentDataPlayerId() == localPlayerId[0].Value)
                predictionMask[index] = 1;
        }
    }
    protected override JobHandle SetPredictedGhostDefaults(NativeArray<ShipSnapshotData> snapshots, NativeArray<int> predictionMask, JobHandle inputDeps)
    {
